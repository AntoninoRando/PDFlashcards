...
function addHeaders(card: IFlashcard, studySet: IStudySet) {
  ...
  while (j >= 0) {
    ...
    const header = lineDescriptor.subParts.find((x) => x.name === "Header");
    if (header === undefined) {
      console.error(
        `"Unable to find header in lienDescriptor: ${lineDescriptor.originalLine}"` +
          JSON.stringify(lineDescriptor.subParts)
      );
    }
    if (lastHeaderLevel !== undefined && header.num >= lastHeaderLevel) {
      break;
    }

    card.headers.unshift(header.text); // Add to beginning to maintain order
    lastHeaderLevel = header.num;
  }
}
